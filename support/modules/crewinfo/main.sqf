				...
				if(!(format["%1", name _x] == "") and !(format["%1", name _x] == "Error: No unit")) then {
					...
					switch(_x)do {
						case commander _vehicle: {
							_text=_text+format ["<t size='1.5'><img image='support\modules\crewinfo\paa\i_commander_ca.paa'></t> <t size='1.35' shadow='true' color='#AAAAFF'>%1</t><br/>", name _x];
						};
	
						case gunner _vehicle: {
							_text=_text+format ["<t size='1.5'><img image='support\modules\crewinfo\paa\i_gunner_ca.paa'></t> <t size='1.35' shadow='true' color='#FF8888'>%1</t><br/>", name _x];
						};
							
						case driver _vehicle: {		
							_text=_text+format ["<t size='1.5'><img image='support\modules\crewinfo\paa\i_driver_ca.paa'></t> <t size='1.35' shadow='true' color='#88FF88'>%1</t><br/>", name _x];
						};
							
						default	{
							if(format["%1", (_role select 0)] != "Turret") then {
								_text=_text+format ["<t size='1.5'><img image='support\modules\crewinfo\paa\i_cargo_ca.paa'></t> <t size='1.35' shadow='true' color='#EEEEEE'>%1</t><br/>", name _x];
							} else {
								_text=_text+format ["<t size='1.5'><img image='support\modules\crewinfo\paa\i_gunner_ca.paa'></t> <t size='1.35' shadow='true' color='#FF8888'>%1</t><br/>", name _x];
							};
						};
