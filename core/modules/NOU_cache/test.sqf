...
diag_log format["MSO-%1 Test NOU Caching Start", time];
...
_fnc_testStat = {
	...
	_verbose = true;
        if(!_param || _verbose) then {
                diag_log format["MSO-%1 Test NOU Caching - %2 - %3", time, _msg, _param];
        };
        _param;
};
...
if(_result) then {
        diag_log format["MSO-%1 Test NOU Caching Successful", time];
} else{ 
        diag_log format["MSO-%1 Test NOU Caching ***Failed***", time];
};
